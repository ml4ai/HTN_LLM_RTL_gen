...
namespace parser {
    ...
    using constant_type = rule<struct TConstant, ast::Constant>;
    BOOST_SPIRIT_DECLARE(constant_type);

    using variable_type = rule<struct TVariable, ast::Variable>;
    BOOST_SPIRIT_DECLARE(variable_type);

    using primitive_type_type = rule<struct TPrimitiveType, ast::PrimitiveType>;
    BOOST_SPIRIT_DECLARE(primitive_type_type);

    using either_type_type = rule<struct TEitherType, ast::EitherType>;
    BOOST_SPIRIT_DECLARE(either_type_type);

    using type_type = rule<struct TType, ast::Type>;
    BOOST_SPIRIT_DECLARE(type_type);

    // Typed list of names
    using explicitly_typed_list_names_type =
        rule<struct TExplicitlyTypedListNames,
    ...
    using typed_list_variables_type =
        rule<struct TTypedListVariables, ast::TypedList<ast::Variable>>;
    BOOST_SPIRIT_DECLARE(typed_list_variables_type);

    using requirements_type =
        rule<struct TRequirements, std::vector<std::string>>;
    BOOST_SPIRIT_DECLARE(requirements_type);

    using atomic_formula_skeleton_type =
        rule<struct TAtomicFormulaSkeleton, ast::AtomicFormulaSkeleton>;
    BOOST_SPIRIT_DECLARE(atomic_formula_skeleton_type);

    using atomic_formula_terms_type =
        rule<struct TAtomicFormulaTerms, ast::AtomicFormula<ast::Term>>;
    BOOST_SPIRIT_DECLARE(atomic_formula_terms_type);

    using literal_terms_type =
        rule<struct TLiteralTerms, ast::Literal<ast::Term>>;
    BOOST_SPIRIT_DECLARE(literal_terms_type);

    using sentence_type = rule<struct TSentence, ast::Sentence>;
    BOOST_SPIRIT_DECLARE(sentence_type);

    using domain_type = rule<struct TDomain, ast::Domain>;
    BOOST_SPIRIT_DECLARE(domain_type);

    using problem_type = rule<struct TProblem, ast::Problem>;
    BOOST_SPIRIT_DECLARE(problem_type);

    using action_type = rule<struct TAction, ast::Action>;
